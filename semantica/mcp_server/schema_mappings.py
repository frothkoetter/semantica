"""
File-based mappings from SQL/Hive schemas to ontology classes in a ContextGraph.

Mapping YAML format (domain-agnostic):

    ontology_namespace: "https://example.org/ontology#"
    tables:
      mytable: MyClass
    columns:
      _global:
...
def ontology_uri(local_name: str, namespace: str) -> str:
    ns = namespace.rstrip("/")
...
def resolve_mapping_config_path(path: Optional[str] = None) -> Optional[str]:
    """Resolve mapping YAML from explicit path or SEMANTICA_MAPPING_CONFIG env."""
    if path:
        return path if os.path.exists(path) else None
    env_path = (os.environ.get("SEMANTICA_MAPPING_CONFIG") or "").strip()
...
def load_mapping_config(path: Optional[str] = None) -> Dict[str, Any]:
    """
    Load ontology↔database mapping YAML.
...
def resolve_table_class(
    ...
    """Resolve table → ontology class URI using explicit mapping config."""
    token = normalize_db_token(table_name)
    class_local = mapping_config.get("tables", {}).get(token)
    if not class_local:
        return None
    ns = mapping_config.get("ontology_namespace", "")
    if not ns:
        return None
    return ontology_uri(class_local, ns), class_local, 1.0
...
def resolve_column_property(
    ...
    """Resolve column → ontology property URI using explicit mapping config."""
    ...
    if not ns:
        return None
    return ontology_uri(prop_local, ns), prop_local, 1.0
...
def apply_explicit_mappings(
    ...
    """Build table/column mapping suggestions using explicit mapping config."""
